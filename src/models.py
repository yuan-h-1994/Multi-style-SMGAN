...
import random
from utils import gaussian, to_var, to_data, save_image, label_input
...
class TextureGenerator(nn.Module):
    ...
    def forward(self, x, labels):
        ...
        out1 = self.model1(x)
        out2 = self.head_0(out1,labels)
        out3 = self.model2(out2)
        out4 = self.head_1(out3,labels)
        out5 = self.model3(out4)
        out6 = self.head_2(out5,labels)
...
class GlyphGenerator(nn.Module):
    ...
    def forward(self, x, l, labels):
        ...
        out3 = self.head_0(out2,labels)
        out4 = self.decoder3(out3)
        out5 = self.head_1(out4,labels)
        out6 = self.decoder5(out5)
        out7 = self.head_2(out6,labels)
...
class ShapeMatchingGAN(nn.Module):
    ...
    def update_structure_discriminator(self, x, xl, l, labels):   
        with torch.no_grad():
            fake_x = self.G_S(xl, l,labels)
...
# Discriminator for crop image
    def update_structure_gtf_discriminator(self, x, xl, l, labels):
    ...
    def update_structure_generator(self, x, xl, l,labels, t=None):
        w = random.randint(0,127)
        h = random.randint(0,127) 
        x_crop = x[:,:,w:w+128,h:h+128]
        fake_x = self.G_S(xl, l,labels)
    ...
    def structure_one_pass(self, x, xl, l, labels, t=None,):
        LDadv = self.update_structure_discriminator(x, xl, l, labels)
        LDgtf = self.update_structure_gtf_discriminator(x, xl, l, labels)
        LGadv, Lrec, LSadv1, Lgly = self.update_structure_generator(x, xl, l, labels, t)
        return [LDadv, LGadv, Lrec, Lgly, LDgtf, LSadv1]    
    
    def update_texture_discriminator(self, x, y, labels):
        with torch.no_grad():
            fake_y = self.G_T(x,labels)          
    ...
    def update_texture_gtf_discriminator(self, x, y, labels):
        w = random.randint(0,127)
        h = random.randint(0,127) 
    ...
    def update_texture_generator(self, x, y, labels, t=None, l=None, VGGfeatures=None, style_targets=None):
        w = random.randint(0,127)
        h = random.randint(0,127) 
        x_crop = x[:,:,w:w+128,h:h+128]
        fake_y = self.G_T(x,labels)
    ...
    def texture_one_pass(self, x, y, labels, t=None, l=None, VGGfeatures=None, style_targets=None):
        LDadv = self.update_texture_discriminator(x, y, labels)
        LDgtf = self.update_texture_gtf_discriminator(x, y, labels)
        LGadv, Lrec,LDadv1, Lsty = self.update_texture_generator(x, y, labels, t, l, VGGfeatures, style_targets)
        return [LDadv, LGadv, Lrec, Lsty, LDgtf, LDadv1]
