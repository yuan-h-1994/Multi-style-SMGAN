...
from utils import load_image, to_data, to_var, visualize, save_image, gaussian,label_input
import os
...
def main():
    ...
    if label == -1:
        ...
        result = []
        while scale <= 1.0: 
            img_str = netGlyph(text, scale,labels) 
            img_str[:,0:1] = torch.clamp(img_str[:,0:1] + noise, -1, 1)
            result1 = netTexture(img_str,labels).detach()
